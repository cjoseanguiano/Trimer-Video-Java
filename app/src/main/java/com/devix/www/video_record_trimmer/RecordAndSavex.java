...
/**
 * Created by Carlos Anguiano on 13/07/17.
 * For more info contact: c.joseanguiano@gmail.com
 */

public class RecordAndSavex extends Activity {
    RecordAndSavex context;
    ...
    private void performAppropriateAction() {
        ...
        int intOperationType = intent.getIntExtra(Utilsx.operation_type, 0);
        String path = "";
        path = intent.getStringExtra(Utilsx.FILE_PATH);
        ...
        switch (intOperationType) {
            ...
            case 4://pass path
                if (path != null) {
                    File file = new File(URI.create(intent.getStringExtra(Utilsx.FILE_PATH)).getPath());
                    if (file.exists() && file.canRead()) {
                        //Do something
                        Log.i("log", "valid path found.");
                        passPath(Uri.parse(intent.getStringExtra(Utilsx.FILE_PATH)));
                    } else {
                        //close activity with error file path not valid or not accessable
                        Log.i("log", "the file path is not accessable or not valid.");
                        finishActivityWithReport(Utilsx.REPORT_TYPE_CANCELLED, "the file path is not accessable or not valid.", Utilsx.REPORT_CODE_FILE_PATH_NOT_VALID);
                    }
                } else {
                    //close activity with error no parameter passed
                    Log.i("log", "please pass a valid file path. do not leave the FILE_PATH empty.");
                    finishActivityWithReport(Utilsx.REPORT_TYPE_CANCELLED, "please pass a valid file path. do not leave the FILE_PATH empty.", Utilsx.REPORT_CODE_NO_FILE_PATH_FOUND);
                }
                break;
            default:
                Log.i("log", "no operation selected.");
                finishActivityWithReport(Utilsx.REPORT_TYPE_CANCELLED, "please select a valid operation type", Utilsx.REPORT_CODE_INVALID_OPERATION_TYPE);
        }
    ...
    public void displayChooserDialog() {
        ...
        dialogChooserDialog.setOnCancelListener(new DialogInterface.OnCancelListener() {
            ...
            public void onCancel(DialogInterface dialogInterface) {
                ...
                finishActivityWithReport(Utilsx.REPORT_TYPE_CANCELLED, "dialog is cancelled", Utilsx.REPORT_CODE_DIALOG_CANCELLED);
            }
        });
    ...
    public void performRecordVideo() {
        ...
        startActivityForResult(videoCapture, Utilsx.REQUEST_VIDEO_TRIMMER);
    ...
    public void performChooseFromGallery() {
        ...
        if (ActivityCompat.checkSelfPermission(context, Manifest.permission.READ_EXTERNAL_STORAGE) != PackageManager.PERMISSION_GRANTED) {
            requestPermission(Manifest.permission.READ_EXTERNAL_STORAGE, context.getString(R.string.permission_read_storage_rationale), Utilsx.REQUEST_STORAGE_READ_ACCESS_PERMISSION);
            ...
            ((Activity) context).startActivityForResult(Intent.createChooser(intent, context.getString(R.string.label_select_video)), Utilsx.REQUEST_VIDEO_TRIMMER);
        }
    ...
    public void onRequestPermissionsResult(int requestCode, @NonNull String[] permissions, @NonNull int[] grantResults) {
        switch (requestCode) {
            case Utilsx.REQUEST_STORAGE_READ_ACCESS_PERMISSION:
    ...
    public void onActivityResult(int requestCode, int resultCode, final Intent data) {
        if (resultCode == RESULT_OK) {
            if (requestCode == Utilsx.REQUEST_VIDEO_TRIMMER) {
                final Uri selectedUri = data.getData();
                if (selectedUri != null) {
                    startTrimActivity(selectedUri);
                } else {
                    Log.i("log", "can not read the file after it is saved to the device memory.");
                    finishActivityWithReport(Utilsx.REPORT_TYPE_CANCELLED, "can not read the file after it is saved to device memory", Utilsx.REPORT_CODE_OUTPUT_FILE_CAN_NOT_BE_READ);
                }
            } else if (requestCode == Utilsx.REQUEST_CODE_RECORD_TO_TRIM) {
                ...
                new AlertDialog.Builder(context)
                        ...
                        .setPositiveButton(android.R.string.yes, new DialogInterface.OnClickListener() {
                            public void onClick(DialogInterface dialog, int which) {
                                Intent intent = new Intent(Intent.ACTION_VIEW, Uri.parse(data.getStringExtra(Utilsx.REPORT)));
                                intent.setDataAndType(Uri.parse(data.getStringExtra(Utilsx.REPORT)), "video/mp4");
                                startActivity(intent);
                                finishActivityWithReport(Utilsx.REPORT__TYPE_DONE, data.getStringExtra(Utilsx.REPORT), Utilsx.REPORT_CODE_SUCCESS);
                            }
                        })
            ...
            if (requestCode == Utilsx.REQUEST_VIDEO_TRIMMER) {
                ...
                new AlertDialog.Builder(context)
                        ...
                        .setNegativeButton(android.R.string.no, new DialogInterface.OnClickListener() {
                            public void onClick(DialogInterface dialog, int which) {
                                finishActivityWithReport(Utilsx.REPORT_TYPE_CANCELLED, "the result is cancelled by android system due to inappropriate selection.", Utilsx.REPORT_CODE_INAPPROPRIATE_SELECTION);
                            }
                        })
                        .setIcon(android.R.drawable.ic_dialog_alert)
                        .show();
            } else if (requestCode == Utilsx.REQUEST_CODE_RECORD_TO_TRIM) {
                Log.i("log", "error in trimming video. error is : " + data.getStringExtra(Utilsx.REPORT));
                if (data.getIntExtra(Utilsx.REPORT_CODE, 0) == Utilsx.REPORT_CODE_TRIMMING_CANCELLED) {
                    finishActivityWithReport(Utilsx.REPORT_TYPE_CANCELLED, data.getStringExtra(Utilsx.REPORT), Utilsx.REPORT_CODE_TRIMMING_CANCELLED);
                } else if (data.getIntExtra(Utilsx.REPORT_CODE, 0) == Utilsx.REPORT_CODE_ERROR_DURING_TRIMMING_VIDEO) {
                    finishActivityWithReport(Utilsx.REPORT_TYPE_CANCELLED, data.getStringExtra(Utilsx.REPORT), Utilsx.REPORT_CODE_ERROR_DURING_TRIMMING_VIDEO);
                }
            }
        }
    }
    ...
    private void startTrimActivity(@NonNull Uri uri) {
        ...
        Intent intent = new Intent(this, TrimmerActivityx.class);
        intent.putExtra(Utilsx.EXTRA_VIDEO_PATH, FileUtils.getPath(this, uri));
        if (intMaxDuration != 0) {
            intent.putExtra(Utilsx.MAX_DURATION, intMaxDuration);
        }
        startActivityForResult(intent, Utilsx.REQUEST_CODE_RECORD_TO_TRIM);
    }
    ...
    private void finishActivityWithReport(int reporttype, String report, int report_code) {
        ...
        if (reporttype == Utilsx.REPORT_TYPE_CANCELLED) {
            setResult(Activity.RESULT_CANCELED, intent);
        } else if (reporttype == Utilsx.REPORT__TYPE_DONE) {
    ...
    public void setMaxDuration() {
        if (getIntent().getIntExtra(Utilsx.MAX_DURATION, 0) != 0) {
            Log.i("log", "the max duration is et to : " + getIntent().getIntExtra(Utilsx.MAX_DURATION, 0));
            intMaxDuration = getIntent().getIntExtra(Utilsx.MAX_DURATION, 0);
        } else {
            Log.i("log", "the max duration is default and the dafault will be 10 seconds. to value was not passed to it.");
        }
    }
}
